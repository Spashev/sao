<template>
  <div>
     <!--<b-button
      v-b-modal.modal-spconns
      variant="default"
      class="float-right btn-link"
      >Добавить spconns</b-button
    >-->
    <b-modal
      id="modal-spconns"
      title="Привязать группу возможных причин к сервису"
      @click="$bvModal.show('bv-modal-example')"
    >
      <form v-on:submit.prevent="onSubmit">
        <label for="staticEmail" class="col-sm-10 col-form-label"
          >Выбрать проблемную группу</label
        >
        <div class="col-sm-10">
          <select
            class="form-control"
...
import {
  ADD_SERVICE_PROBLEM_CONNS,
  SET_SERVICE_PROBLEM_GROUPS,
...
export default {
  ...
  methods: {
    async onSubmit() {
      ...
      //  }
      await this.$store.dispatch(ADD_SERVICE_PROBLEM_CONNS, formData, config);
      let self = this;
