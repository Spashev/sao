<template>
  <div class="container-fluid">
      <Menu/>
      <h4>Uplink data traffic volume (GB)</h4>
    <div class="row">
...
export default {
  ...
  methods: {
    ...
    getBySettl(sett) {
      axios.get(`http://192.168.212.2:8009/dashboard/kpi_3g_city/`+this.type+`/`+this.settl1).then((response) => {
    ...
    getByDate(data) {
      ...
      const reformatedDate = moment(settldate).format('DD/MM/YYYY');
       axios.get(`http://192.168.212.2:8009/dashboard/top_10_3g_cells/`+this.type+`/`+this.settl1+'?date='+reformatedDate).then((response) => {
