<template>
  <div class="container-fluid">
    <Menu />
    <h4>Downlink data traffic volume (GB)</h4>
    <div class="row">
      <div class="col-sm-12"><BarChart @on-receive="defineSettl" /></div>
      <div class="col-sm-12">
        <LineChart
          @on-receive="getByDate"
          :chartdata="settl1_data"
          v-if="settl1_data"
          chart:update
        />
      </div>
...
export default {
  ...
  methods: {
    ...
    getBySettl(sett) {
      axios
        .get(
          `http://192.168.212.2:8009/dashboard/kpi_4g_city/` +
            this.type +
            `/` +
            this.settl1
        )
    ...
    getByDate(data) {
      ...
      axios
        .get(
          `http://192.168.212.2:8009/dashboard/top_10_4g_cells/` +
            this.type +
            `/` +
            this.settl1 +
            "?date=" +
